...
class PaymentApp:
    ...
    def select_menu(self, menu):
        ...
        current_order = self.order_entry.get("0.0", tk.END).strip()
    ...
    def pay_and_clear(self):
        self.pay()
        self.order_entry.delete("0.0", tk.END)  # "Your order:" 초기화

    def pay(self):
        selected_menu = self.selected_menu.get()
        order = self.order_entry.get("0.0", tk.END).strip()
        ...
        self.purchase_history.insert(tk.END, f"{order} - ₩{self.total_price:,}\n")

        self.total_price = 0  # 지불 후 누적 가격 초기화

    def clear_history(self):
        self.purchase_history.delete(0.0, tk.END)  # 구매 내역 초기화
